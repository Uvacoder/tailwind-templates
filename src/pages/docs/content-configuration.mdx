...
```html
<div class="text-{{ error ? 'red' : 'teal' }}-600"></div>
```

In the example above, the strings `text-red-600` and `text-teal-600` do not exist, so Tailwind will not generate those classes.
...
```html
<div class="{{ error ? 'text-red-600' : 'text-teal-600' }}"></div>
```
...
module.exports = {
  ...
  safelist: [
    ...
    'text-3xl',
    {
      pattern: /bg-(red|teal|blue)-(100|200|300)/,
    },
  ],
  // ...
}
